...
function printOrder(order){
  try {
    ...
    w.document.write(html); w.document.close();
  } catch(e){ console.error(e); alert("Falha ao imprimir."); }
}
  ...
  try {
    ...
    rows.forEach(function(tr){
      ...
      btn.addEventListener("click", async function(){
        var ord = await fetchOrderById(id);
        printOrder(ord);
      });
      actionsTd.appendChild(btn);
      tr.setAttribute("data-print-bound","1");
    });
  } catch(e){ console.warn("attachPrintButtons", e); }
}

window.addEventListener("load", function(){
  try { attachPrintButtons(); setInterval(attachPrintButtons, 2000); } catch(e){}
});
